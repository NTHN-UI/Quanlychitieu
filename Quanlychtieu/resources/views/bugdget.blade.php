...
<body>
    ...
    <script>
        ...
        const generateForecast = () => {
            ...
            if (Object.keys(lastMonthExpenses).length === 0) {
                forecastList.innerHTML = '<li class="list-group-item">Không có dữ liệu dự báo</li>';
                return;
            }

            for (const [category, amount] of Object.entries(lastMonthExpenses)) {
                const expenseAmount = parseFloat(amount.replace(/,/g, '')) || 0;
                const forecastAmount = expenseAmount; // Tùy chỉnh dự báo
        ...
        document.addEventListener('DOMContentLoaded', () => {
            generateForecast(); // Gọi khi tải trang
        });
